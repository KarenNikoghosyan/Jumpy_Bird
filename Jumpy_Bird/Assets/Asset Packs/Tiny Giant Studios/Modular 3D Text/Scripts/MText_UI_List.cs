    ...
    public class MText_UI_List : MonoBehaviour
    {
        ...
        bool IsItemSelectable(int i)
        {
            if (transform.childCount > i)
            {
                ...
                //it's a input Field
                else if (target.GetComponent<Mtext_UI_InputString>())
                    return target.GetComponent<Mtext_UI_InputString>().interactable;
        ...
        void AlertSelectedItem(int number)
        {
            if (transform.childCount > number)
            {
                ...
                itemTransform.GetComponent<Mtext_UI_InputString>()?.Focus(true);
