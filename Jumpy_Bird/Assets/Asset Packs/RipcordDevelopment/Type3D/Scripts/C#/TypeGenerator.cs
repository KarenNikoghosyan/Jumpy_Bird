...
public class TypeGenerator : MonoBehaviour {
	...
	public void GenerateType () {
		...
		for (int y = 0; y < type3DList.Count; y++) {
			GameObject newCharacter = (GameObject)Instantiate(type3DList[y], transform.position, transform.rotation);
	...
	public void SaveType3D () {
		...
		#if UNITY_EDITOR
		if (!Directory.Exists("Assets/RipcordDevelopment/Type3D/Resources")) {									//If the Type3D resources folder does not exist...
			Directory.CreateDirectory("Assets/RipcordDevelopment/Type3D/Resources");							//...create the Type3D resources folder
		}
			
		if (type3DContainer) {																					//If there is an existing Type3D container in the scene...

			UnityEngine.Object newPrefab = PrefabUtility.CreateEmptyPrefab("Assets/RipcordDevelopment/Type3D/Resources/" + "NewType3D" + ".prefab");
			PrefabUtility.ReplacePrefab(type3DContainer, newPrefab, ReplacePrefabOptions.ConnectToPrefab);

			GameObject newType3D = Resources.Load("NewType3D") as GameObject;									//Create a reference to the Type3D container that was just saved
			...
			foreach (Transform child in newType3D.transform) {													//Cycle through each character in the saved Type3D container...
				...
				Vector3 resetPosition = child.transform.localPosition;
				child.transform.localPosition = new Vector3(resetPosition.x, 0.0f, resetPosition.z);			//...reset the y position of the character...
				child.transform.eulerAngles = Vector3.zero;														//...reset the rotation of the character...
